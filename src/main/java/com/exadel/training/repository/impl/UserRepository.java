...
public interface UserRepository extends JpaRepository<User,Long>{
  ...
  //  @Query("select u from User u where u.login = ?1 AND u.password = ?2")
   ...
   @Query(value = "insert into users_trainings values(:trainingID,:userID)", nativeQuery = true)
    ...
    @Query("select distinct t from User as u inner join u.trainings as t inner join t.coach as c where u.login = ?1 and t.state in (?2) and c.id = u.id order by t.dateTime asc")
